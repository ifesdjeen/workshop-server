...
(defn sandbox*
  ...
  (let [timeout       (or timeout 10000)
        namespace     (or namespace (gensym "sandbox"))
        context       (or context (-> (permissions) clojail.jvm/domain clojail.jvm/context))
        jvm           (or jvm true)
        transform     (or transform eagerly-consume)
        refer-clojure (or refer-clojure true)
        max-defs      (or max-defs 50)
        nspace        (create-ns namespace)]
    (binding [*ns* nspace]
